...
#' \item{Z}{the wavelet basis functions design matrix when \code{Hybrid=F}}
#'
#' @examples
...
GAMwv <- function(y,A,numlev=6,Hybrid=FALSE, penalty="SCAD",
                  ...
                  conv.thresh=1.e-10, offset=0, max.iter=ncol(A)*5,trace=FALSE, nfolds=10)
{
  ...
  while(continue & (iter<max.iter)){
    ...
    for(pp in 1:p){
	  ...
	  if (Hybrid) {
		  fit<- fitHybrid(x,y,eps=0.001,maxiter=30,penalty=penalty,numlevels=numlev)
		  Shatp<-fit$fit
		  betaHat[,pp]<-fit$beta
	  }
	  if (!Hybrid){ 
