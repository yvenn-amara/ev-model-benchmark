...
## shifting sessions arriving simultaneously
# rounds = 0
# while(sum(duplicated(df$Start))!=0){
#   rounds = rounds +1
# 
#   df$Start[duplicated(df$Start)] = df$Start[duplicated(df$Start)] + 60 # shift sessions arriving at the same time
#   print(paste("Remaining simultaneous arrivals to be shifted:",sum(duplicated(df$Start))))
# }

# window = 60 #60  # size of sliding window in minutes
filter = array(1/window, dim=window)
boundaries = read.csv(paste("2. R/1. Outputs/",dataset,"/boundaries.csv",sep=""))
